...
| `<save_id>/production.json` | every `flma-tick-interval` (full overwrite) | per-force, per-surface item/fluid `input_counts`/`output_counts` (lifetime cumulative totals) **and** `input_rates_per_<window>`/`output_rates_per_<window>` for `window` in `min`/`10min`/`hour`/`10hr` (real per-minute flow rates via `get_flow_count`, each averaged over a different lookback so a genuine trend can be told apart from a momentary blip) |
